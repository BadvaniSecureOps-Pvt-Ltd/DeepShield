...
WEIGHTS_PATH_DEFAULT = "app/models/weights/deepfake_detector_best.pth"
...
TEMPERATURE = 1.5  # reduces softmax overconfidence
...
def predict(file_bytes: bytes, filename: str = None):
    try:
        ...
        with torch.no_grad():
            ...
            # Debug: raw logits for both classes
            raw_logits = outputs[0].cpu().numpy()
            print("RAW OUTPUT LOGITS:", raw_logits)

            # ---- Class decision based purely on logits (correct choice) ----
            pred_idx = int(np.argmax(raw_logits))  # 0 = deepfake, 1 = real

            # ---- Probability calculation using temperature scaling ----
            scaled_probs = torch.softmax(outputs / TEMPERATURE, dim=1)[0]
            confidence = float(scaled_probs[pred_idx].item())
